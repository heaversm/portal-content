...
There are several ways to upload images via the thirdweb SDK for minting NFTs, in this guide we will go over them in detail and how they work, more specifically using the TypeScript SDK and the Python SDK.
...
We'll cover all the ways in which we can do it, with details along with implementation and all the info you want to know about it, with both Python and Typescript, let's go! 🔥
...
You can upload JPEGs, PNGs, GIFs, and other common image formats. In fact you can upload _any_ file that you like, audio, video, html, text, pdf, and 3d model files.

Let's go through each option in detail.

## 1. Local file stored on your computer

This is a nifty way to upload images to the Thirdweb SDK, by passing the image file, thirdweb will take care of uploading to IPFS behind the scenes, and mint the NFT out of it.
...
// Get the module
const nftCollection = sdk.getNFTModule("<NFT_COLLECTION_MODULE_ADDRESS>");
...
nft_collection_module.mint(MintArg(
    name=name_nft,
...
### 2. Create the form with Thirdweb SDK.
...
const FrontendForm = () => {
  ...
  const formSubmit = (ev) => {
    ...
    if (image) {
      (async () => {
        console.log(
          await nftCollectionModule.mint({
            name: "Thirdweb NFT.",
...
nft_collection_module_address = "0xD91A8C3Dd5fa4F829A009FCd9C1DDc8417DB78f9"
...
![Screenshot of a web app for uploading images to ipfs](./screenshot.png)
...
That is it! You now know how to correctly upload image files to the thirdweb Python and TypeScript SDK!
